...
- `api/chat.js` — streaming chat-endpoint. Gemini 2.5 Flash + function calling. Zeven tools:
  ...
  - `find_team_members({skill, technology, sector, seniority, available_only, keyword})` — filtert consultants in de `team_members`-tabel voor team-match-vragen. Substring-match op skills/technologies, exacte match op sector (canonical lijst), keyword zoekt door naam/rol/summary/projectervaring/certificaten. Top 8 matches met gestructureerde profielen — geen raw `cv_text` (privacy + token-budget). Vector-search op cv_text staat op de roadmap (Fase C).
  - `get_team_member({name})` — volledig profiel van één consultant op naam (fuzzy match). Voor klantgerichte profielpitch of diepere blik op één teamlid. Retourneert geen cv_text/cv_pdf-info.
...
- **Team-feature Fase A (branch `nova-team-profiles`, gemerged in main):** nieuwe Beheer sub-tab "Team" voor consultant-profielen. Database: `team_members`-tabel met gestructureerde velden (name, role, seniority, kernskills/technologies/sectors als text[], project_experience jsonb, certifications, summary, current_client, available_from) + Storage-bucket `team-cvs` voor de CV-bestanden. Beide RLS authenticated-all (intern team-data, geen owner-koppeling in Fase A). Frontend: `TeamManager` (lijst + acties), `TeamMemberEditor` (form). Twee upload-flows: "+ Nieuw teamlid" voor handmatig + "+ CV uploaden" voor PDF/DOCX → parse → prefill. Server-endpoint `api/cv-parse.js` doet `unpdf` voor PDF-extractie of `mammoth` voor DOCX + Gemini 2.5 Flash met `responseSchema` voor structured output. Pad-conventie: bestanden in Storage onder `<member-id>/<timestamp>-<filename>`. **Bug-geschiedenis:** initieel `pdf-parse` v2 had module-load-bug op Vercel serverless (probeert bij import een test-PDF te lezen) → switched naar `unpdf` (serverless-friendly). DOCX toegevoegd omdat image-zware Canva/Pages PDFs maar 193 chars opleverden — Word-export is betrouwbaarder.
- **Team-feature Fase B (gemerged):** Nova kan teamleden zoeken + voorstellen. Twee nieuwe tools in `api/chat.js`:
  - `find_team_members({skill, technology, sector, seniority, available_only, keyword})` — filtert `team_members`. Substring-match op skills/technologies, exacte match op canonical sector-lijst, keyword zoekt door naam/rol/summary/projectervaring/certificaten. Top 8 matches gestructureerd, géén raw `cv_text` in de response (privacy + token-budget).
  - `get_team_member({name})` — volledig profiel op naam (fuzzy match) voor klantgerichte profielpitch.
  - Bucket-aware availability filter: `available_only=true` accepteert leden zónder huidige klant **of** met `available_from <= today`. ChatPanel-chips: `find_team_members` → Team, `get_team_member` → Profiel.
- **Branch-sync cases ↔ team-leden:** `app_config.branches` is nu single source of truth voor sectoren in zowel `CaseEditor` (mapping.branches) als `TeamMemberEditor` (sectors). 13 canonical sectoren incl. nieuwe Telecom & media, Bouw & vastgoed, Agri & food, Cultuur & recreatie. `listBranches()` in `src/lib/teamMembers.js` leest 'm. Gemini's `responseSchema` voor CV-parse heeft een enum-constraint op deze lijst zodat extractie geen vrije teksten produceert.
- **Senioriteits-schaal Creates-eigen:** `['Starter', 'Young Professional', 'Professional', 'Senior', 'Expert']`. Editor heeft een `SENIORITY_MIGRATION`-map die oude waardes (Junior/Medior/Lead/Principal) naar deze schaal mapt zodat bestaande profielen niet stuk gaan.
- **Gids-startpagina — Beschikbaarheid team-strip:** `TeamGrid` rendert bovenaan de Gids-route met titel "Beschikbaarheid team — N". Kanban-layout: buckets als parallelle kolommen via CSS grid (`auto-fill, minmax(220px, 1fr)`); cards stacken vertikaal in elke bucket, op mobiel collapsen ze onder elkaar. Buckets:
  - **Nu beschikbaar** (geen huidige klant of `available_from <= today`)
  - **Vrij in [maand jaar]** — maand-bucket per kalendermaand, tot 6 maanden vooruit
  - **Later** (>6 maanden)
  - **Bezet — einddatum onbekend** (klant ingevuld, geen `available_from`)
  - Bucket-logica in `getAvailabilityBucket(member)` / `groupTeamByAvailability(members)` in `src/lib/teamMembers.js` — gedeeld met Beheer-list (badge per row).
  - Click op card → read-only `TeamMemberDetail`-modal (signed Storage-URL met 600s TTL voor CV-preview).
- **Gids cards-cleanup:** TeamGrid-cards en CasesOverview-cards hadden te veel chips (skills, doelen/behoeften/diensten) — visueel druk in 't startpagina-grid. Verwijderd uit beide. CasesOverview behoudt branche-chip + persona-badges (= identiteit op één regel); strategische mapping-tags zijn beschikbaar zodra je de card uitklapt naar `ReferenceCard`. Beide secties hebben nu dezelfde header-stijl: "**Cases** — N" / "**Beschikbaarheid team** — N".
- **Cross-origin download fix:** HTML `download` attribute werkt niet voor cross-origin URLs (Supabase Storage zit op andere domain). Editor gebruikt nu blob-fetch + temporary anchor met same-origin object-URL voor "PDF downloaden", terwijl "PDF bekijken" gewoon de signed URL opent.
- **Volgende werk (Team-feature, in volgorde van impact):**
  - **Fase C** — pgvector embeddings voor semantische CV-zoek (alternatief voor substring-match, vooral voor zachtere skill-vragen).
  - **Fase D** — bidirectionele mapping cases ↔ team-leden (welke consultants werkten op welke case, welke cases match een profiel).
  - **Fase E** — sales-enablement output-format: top-3-profiel-blok + relevante cases + klantgerichte pitch + gap-flag + conceptmail in één Nova-respons.
- **DEPLOY-NOTE (productie Supabase, idempotent):**
  1. `alter table public.team_members add column if not exists available_from date;` — vervangt eerdere binary `available_for_sales` (column blijft staan, niet meer gebruikt).
  2. `alter table public.team_members add column if not exists current_client text;` — als nog niet eerder gedraaid.
