...
- **Team-feature Fase B (gemerged):** Nova kan teamleden zoeken + voorstellen. Twee nieuwe tools in `api/chat.js`:
  - `find_team_members({skill, technology, sector, seniority, available_now, keyword})` — filtert `team_members`. Substring-match op skills/technologies, exacte match op canonical sector-lijst, keyword zoekt door naam/rol/summary/projectervaring/certificaten. Top 8 matches gestructureerd, géén raw `cv_text` in de response (privacy + token-budget).
  - `get_team_member({name})` — volledig profiel op naam (fuzzy match) voor klantgerichte profielpitch.
  - Bucket-aware availability filter: `available_now=true` accepteert leden zónder huidige klant **of** met `available_from <= today`. ChatPanel-chips: `find_team_members` → Team, `get_team_member` → Profiel.
